...
configure_network_bbr() {
    ...
    SYSCTL_SETTINGS=(
        ...
        "net.core.netdev_max_backlog=12000"
...
<body>
    <div class="container">
        ...
        <p>If you're seeing this page, it means Nginx is successfully installed and running on your Debian server. You can now configure Nginx to serve your web applications or other services.</p>
        <p>For further configuration and documentation, please refer to <a href="http://nginx.org/" target="_blank">nginx.org</a>.</p>
        <a href="https://www.debian.org/" class="button" target="_blank">Visit Debian Official Website</a>
    </div>
    <div class="footer">
        <p>Powered by <a href="https://www.debian.org/" target="_blank">Debian</a> & <a href="https://nginx.org/" target="_blank">Nginx</a></p>
    </div>
</body>
</html>
EOF
...
EOF
    ...
    if [[ "$current_mode" == "wal" ]]; then
        ...
        enable_wal_result=$(docker exec marzban python3 - << 'EOF'
import sqlite3
db = sqlite3.connect("/var/lib/marzban/db.sqlite3")
...
EOF
        ...
        if [[ $? -eq 0 ]]; then
            ...
            log red "Failed to enable SQLite WAL mode: $enable_wal_result"
        fi
    fi
...
EOF
    ...
    if [[ "$answer" != "n" && "$answer" != "N" ]]; then
        ...
        cat /dev/null > /root/.bash_history && history -c && rm -f "/root/install.sh"
        reboot
    fi
}
