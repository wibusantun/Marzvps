...
install_packages() {
    ...
    apt-get update -y || log yellow "Warning: Failed to fully update package lists, continuing..."
    ...
    apt-get install -y \
        perl libio-socket-ssl-perl libwww-perl \
        libpcre3 libpcre3-dev zlib1g-dev dbus iftop zip unzip wget \
        net-tools nano sed screen gnupg bc build-essential \
        dirmngr dnsutils at htop iptables bsdmainutils cron \
        lsof lnav jq sqlite3 libsqlite3-dev inotify-tools \
...
install_docker_debian_10() {
    if [[ -f /etc/os-release ]]; then
        source /etc/os-release
        if [[ "$ID" == "debian" && "$VERSION_ID" == "10" ]]; then
            log blue "Debian 10 detected. Installing Docker manually from Buster repository..."
            apt-get install -y apt-transport-https ca-certificates curl gnupg2 software-properties-common || true
            curl -fsSL https://download.docker.com/linux/debian/gpg | apt-key add -
            add-apt-repository "deb [arch=amd64] https://download.docker.com/linux/debian buster stable"
            apt-get update -y || log yellow "Warning: apt update issue on Debian 10, continuing..."
            apt-get install -y docker-ce docker-ce-cli containerd.io docker-compose-plugin || { log red "Failed to install Docker for Debian 10."; exit 1; }
            log green "Docker installed successfully for Debian 10."
        fi
    fi
}
...
main() {
    ...
    timedatectl set-ntp true || log yellow "Failed to enable NTP, skipping..."
	# Install Docker Debian 10
    install_docker_debian_10
    # === Install Marzban ===
...
services:
  marzban:
    ...
    volumes:
      ...
      - /var/run/marzban:/var/run/marzban
  ...
  nginx:
    ...
    volumes:
      ...
      - /var/lib/marzban/log/nginx:/var/log/nginx
...
EOF
    ...
    mkdir -p /var/www/html /var/run/marzban/ /var/lib/marzban/log/nginx
    touch /var/lib/marzban/log/nginx/access.log
    wget -qO /tmp/html.zip "https://raw.githubusercontent.com/wibusantun/Marzvps/main/html.zip"
    unzip -oq /tmp/html.zip -d /var/www/
    rm -f /tmp/html.zip
    chown -R www-data:www-data /var/www/html
    chmod -R 755 /var/www/html /var/lib/marzban/log
    chmod -R 755 /var/run/marzban
...
http {
    ...
    upstream grpc_trojan_backend {
        server unix:/var/run/marzban/trojan_grpc.sock;
        keepalive 64;
    }
    upstream grpc_vmess_backend {
        server unix:/var/run/marzban/vmess_grpc.sock;
        keepalive 64;
    }
    upstream grpc_vless_backend {
        server unix:/var/run/marzban/vless_grpc.sock;
        keepalive 64;
    }
    ...
    server {
        ...
        location = /trojan {
            ...
            proxy_redirect                   off;
            proxy_pass                       http://unix:/var/run/marzban/trojan_ws.sock:/trojan;
        ...
        location = /vmess {
            ...
            proxy_request_buffering          off;
            proxy_redirect                   off;
            proxy_pass                       http://unix:/var/run/marzban/vmess_ws.sock:/vmess;
            proxy_http_version               1.1;
            proxy_set_header Host            $host;
            proxy_set_header Upgrade         $http_upgrade;
            proxy_set_header Connection      $connection_upgrade;
        ...
        location = /vless {
            ...
            proxy_request_buffering          off;
            proxy_redirect                   off;
            proxy_pass                       http://unix:/var/run/marzban/vless_ws.sock:/vless;
            proxy_http_version               1.1;
            proxy_set_header Host            $host;
            proxy_set_header Upgrade         $http_upgrade;
            proxy_set_header Connection      $connection_upgrade;
        ...
        location = /uptrojan {
            ...
            proxy_request_buffering          off;
            proxy_redirect                   off;
            proxy_pass                       http://unix:/var/run/marzban/trojan_up.sock:/uptrojan;
            proxy_http_version               1.1;
            proxy_set_header Host            $host;
            proxy_set_header Upgrade         $http_upgrade;
            proxy_set_header Connection      $connection_upgrade;
        ...
        location = /upvmess {
            ...
            proxy_request_buffering          off;
            proxy_redirect                   off;
            proxy_pass                       http://unix:/var/run/marzban/vmess_up.sock:/upvmess;
            proxy_http_version               1.1;
            proxy_set_header Host            $host;
            proxy_set_header Upgrade         $http_upgrade;
            proxy_set_header Connection      $connection_upgrade;
        ...
        location = /upvless {
            ...
            proxy_request_buffering          off;
            proxy_redirect                   off;
            proxy_pass                       http://unix:/var/run/marzban/vless_up.sock:/upvless;
            proxy_http_version               1.1;
            proxy_set_header Host            $host;
            proxy_set_header Upgrade         $http_upgrade;
            proxy_set_header Connection      $connection_upgrade;
        ...
        location /grpc-trojan {
            ...
            grpc_pass                        grpc://grpc_trojan_backend;
        ...
        location /grpc-vmess {
            ...
            grpc_socket_keepalive            on;
            grpc_pass                        grpc://grpc_vmess_backend;
            grpc_set_header X-Real-IP        $remote_addr;
            grpc_read_timeout                1h;
            grpc_send_timeout                1h;
            client_body_timeout              1h;
        ...
        location /grpc-vless {
            ...
            grpc_socket_keepalive            on;
            grpc_pass                        grpc://grpc_vless_backend;
            grpc_set_header X-Real-IP        $remote_addr;
            grpc_read_timeout                1h;
            grpc_send_timeout                1h;
            client_body_timeout              1h;
...
EOF
    ...
    # === Install Marzban Socket Watcher ===
    log blue "Setting up Marzban Socket Watcher..."
    cat > /usr/local/bin/marzban-watcher.sh << 'EOF'
#!/bin/bash
TARGET_DIR="/var/run/marzban"

mkdir -p "$TARGET_DIR"
chmod 755 "$TARGET_DIR"

chmod 777 $TARGET_DIR/*.sock 2>/dev/null

inotifywait -m "$TARGET_DIR" -e create | while read -r directory event file; do
    if [[ "$file" == *.sock ]]; then
        chmod 777 "$TARGET_DIR/$file"
    fi
done
EOF

    chmod +x /usr/local/bin/marzban-watcher.sh
    cat > /etc/systemd/system/marzban-watcher.service << 'EOF'
[Unit]
Description=Automated Marzban Socket Permissions Watcher
After=network.target

[Service]
Type=simple
ExecStart=/usr/local/bin/marzban-watcher.sh
Restart=always
RestartSec=3
User=root
...
EOF
    ...
    systemctl enable marzban-watcher.service
    systemctl start marzban-watcher.service

    log green "Marzban Socket Watcher setup complete and running."
    ...
    apt install -y iptables curl socat xz-utils apt-transport-https gnupg gnupg2 dnsutils lsb-release cron bash-completion || { log red "Failed to install socat packages."; exit 1; }
	...
	# === Generating Xray Reality Keys ===
	log blue "Generating Xray Reality Keys..."
	REALITY_KEYS=$(/var/lib/marzban/xray-core/xray x25519)
	PRIVATE_KEY=$(echo "$REALITY_KEYS" | grep "Private key:" | awk '{print $3}')
	PUBLIC_KEY=$(echo "$REALITY_KEYS" | grep "Public key:" | awk '{print $3}')
	log info "Reality Public Key: $PUBLIC_KEY"
	echo "Reality Public Key: $PUBLIC_KEY" >> "$LOG_FILE"
  ...
  "inbounds": [
    {
      ...
      "streamSettings": {
        ...
        "realitySettings": {
          ...
          "privateKey": "KEfVlcdR0JCJy8rZHYvNgXAUnXB2rxBAWq-IrmazLXA",
          "shortIds": [
            ""
          ]
        }
    ...
    {
      "tag": "TROJAN_WS",
      "listen": "/var/run/marzban/trojan_ws.sock",
      "port": 3001,
      "protocol": "trojan",
      "settings": {
        "clients": []
    ...
    {
      "tag": "VMESS_WS",
      "listen": "/var/run/marzban/vmess_ws.sock",
      "port": 3002,
      "protocol": "vmess",
      "settings": {
        "clients": []
    ...
    {
      "tag": "VLESS_WS",
      "listen": "/var/run/marzban/vless_ws.sock",
      "port": 3003,
      "protocol": "vless",
      "settings": {
        "clients": [],
    ...
    {
      "tag": "TROJAN_UPGRADE",
      "listen": "/var/run/marzban/trojan_up.sock",
      "port": 3004,
      "protocol": "trojan",
      "settings": {
        "clients": []
    ...
    {
      "tag": "VMESS_UPGRADE",
      "listen": "/var/run/marzban/vmess_up.sock",
      "port": 3005,
      "protocol": "vmess",
      "settings": {
        "clients": []
    ...
    {
      "tag": "VLESS_UPGRADE",
      "listen": "/var/run/marzban/vless_up.sock",
      "port": 3006,
      "protocol": "vless",
      "settings": {
        "clients": [],
    ...
    {
      "tag": "TROJAN_GRPC",
      "listen": "/var/run/marzban/trojan_grpc.sock",
      "port": 3007,
      "protocol": "trojan",
      "settings": {
        "clients": []
    ...
    {
      "tag": "VMESS_GRPC",
      "listen": "/var/run/marzban/vmess_grpc.sock",
      "port": 3008,
      "protocol": "vmess",
      "settings": {
        "clients": []
    ...
    {
      "tag": "VLESS_GRPC",
      "listen": "/var/run/marzban/vless_grpc.sock",
      "port": 3009,
      "protocol": "vless",
      "settings": {
        "clients": [],
...
EOF
    ...
    # === Inject Private Key to JSON ===
    sed -i "s/PRIVATE_KEY/$PRIVATE_KEY/" /var/lib/marzban/xray_config.json
	# === Finalize Marzban ===
    ...
    get_token() {
        ...
        for i in {1..30}; do
            token_json=$(curl -sf -X POST "http://127.0.0.1:8000/api/admin/token" \
	...
	u="URL: https://${domain}/dashboard"
	usr="Username: ${userpanel}"
	pw="Password: ${passpanel}"
	pub="Reality Public Key: ${PUBLIC_KEY}"
